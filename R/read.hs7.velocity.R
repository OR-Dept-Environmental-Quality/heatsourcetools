#' Read Heat Source 7 flow velocity outputs.
#'
#' Read flow velocity outputs from the hydraulics output worksheet in a Heat
#' Source version 7 model. The Heat Source excel workbook needs to be saved as
#' .xlsm. Workbooks in .xls do not seem to work.
#'
...
read.hs7.velocity <- function(output_dir, file_name, sheet_name = "Output - Hydraulics") {
  ...
  col_names <- c("model_km",
                 paste0("X_Flow", seq(from = model_date, to = model_date + sim_days - 1)),
                 "X1",
                 seq(from = model_date, to = model_date + sim_days - 1),
                 "X2",
                 paste0("X_Wetted_Width", seq(from = model_date, to = model_date + sim_days - 1)),
                 "X3",
                 paste0("X_Ave_Wetted_Depth", seq(from = model_date, to = model_date + sim_days - 1)),
                 "X4",
                 paste0("X_Max_Wetted_Depth", seq(from = model_date, to = model_date + sim_days - 1)),
                 "X5",
                 paste0("X_Hyporheic_Exchange", seq(from = model_date, to = model_date + sim_days - 1)),
                 "X6",
                 paste0("X_Tendency_to_Stratify", seq(from = model_date, to = model_date + sim_days - 1)))

  excel.data <- readxl::read_excel(path = file.path(output_dir, file_name),
                                   sheet = sheet_name,
                                   range = cellranger::cell_limits(c(17, 5), c(NA, NA)),
                                   col_names = col_names,
                                   na = c("","N/A", " ")) %>%
    dplyr::select(model_km, !dplyr::starts_with("X"))
